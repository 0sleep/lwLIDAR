# From https://github.com/ferrolho/VL53L5CX-BNO08X-viewer/blob/main/viewer/geometry.py
...
def compute_zone_angles() -> ZoneAngles:
    ...
    pitch_rad = np.deg2rad(ST_PITCH_ANGLES_DEG)
    yaw_rad = np.deg2rad(ST_YAW_ANGLES_DEG)
...
def get_colors(distances: np.ndarray, status: np.ndarray) -> np.ndarray:
    ...
    d_norm = np.clip(
        (distances - MIN_RANGE_MM) / (MAX_RANGE_MM - MIN_RANGE_MM),
        0,
        1,
    )

    # Status 5 = valid measurement
    valid = (status == 5) & (distances >= MIN_RANGE_MM)
