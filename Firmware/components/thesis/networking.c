...
static esp_netif_t *networking_netif = NULL;
...
esp_err_t networking_init(void)
{
    ...
    networking_netif = esp_netif_create_default_wifi_sta();
    if (networking_netif == NULL) {
...
esp_err_t networking_connect(char* wifi_ssid, char* wifi_password)
{
...
esp_err_t networking_disconnect(void)
{
...
esp_err_t networking_deinit(void)
{
    ...
    ESP_ERROR_CHECK(esp_wifi_clear_default_wifi_driver_and_handlers(networking_netif));
    esp_netif_destroy(networking_netif);
